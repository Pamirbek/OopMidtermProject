...
public class Registration extends JFrame implements ActionListener {
    ...
    JButton[] totalBtns = new JButton[students.size()];
    ...
    Registration(){
        ...
        for (int i = 0; i < students.size(); i++) {
            ...
            totalStudents[i].setBounds(110, 480 + (i*25), 500, 30);
            add(totalStudents[i]);
        }

        for (int i = 0; i < students.size(); i++) {
            totalIDs[i] = new JLabel("ID: " + students.get(i).getId());
            totalIDs[i].setBounds(300, 480 + (i*25), 500, 30);
            add(totalIDs[i]);
        }

        for (int i = 0; i < students.size(); i++) {
            totalBtns[i] = new JButton("Delete " + (i+1));
            totalBtns[i].addActionListener(this);
            totalBtns[i].setBounds(490, 480 + (i*25), 80, 30);
            add(totalBtns[i]);
        }
    ...
    private void deleteBtn(int i) throws IOException {
        File inputFile = new File("data_base.txt");
        File tempFile = new File("temp_data_base.txt");

        BufferedReader reader = new BufferedReader(new FileReader(inputFile));
        BufferedWriter writer = new BufferedWriter(new FileWriter(tempFile));

        int lineToRemove = i*7;
        int nLine = 0;
        String currentLine;

        while((currentLine = reader.readLine()) != null) {

            if (nLine == lineToRemove) {
                reader.readLine();
                reader.readLine();
                reader.readLine();
                reader.readLine();
